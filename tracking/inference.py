...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = [i for i in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)] 
    variableDomainsDict[OBS1] = [i for i in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)] 
    "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            ...
            for newPosition in newPosDist:
                updatedBeliefs[newPosition] += newPosDist[newPosition] * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            pos = self.legalPositions[i % len(self.legalPositions)]  # use mod to wrap around overflow
